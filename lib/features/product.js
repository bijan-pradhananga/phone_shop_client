...
export const addViewedProducts = createAsyncThunk(
    ...
    async ({ productId, userId }, { rejectWithValue }) => {
      try {
        // Send GET request with query parameters for pagination and sorting
        const response = await API.post(`activity/view`,{ productId, userId });
...
export const fetchSimilarProducts = createAsyncThunk(
    'product/similar',
    async ({ phone_model , limit = 4 }, { rejectWithValue }) => {
      try {
        // Send GET request with query parameters for pagination and sorting
        const response = await API.get(`product/similar?phone_model=${phone_model}&top_n=${limit}`);
        return response.data; // Returns products, total, currentPage, totalPages
      } catch (error) {
        return handleError(error, rejectWithValue); // Custom error handling function
      }
...
export const fetchRecommendedProducts = createAsyncThunk(
    ...
    async ({ userId, limit = 4 }, { rejectWithValue }) => {
      try {
        ...
        builder
            ...
            .addCase(fetchRecommendedProducts.pending, (state) => {
              ...
              .addCase(fetchRecommendedProducts.fulfilled, (state, action) => {
                state.isLoading = false;
                state.recommendedData = action.payload.products;
                state.error = null;
              })
              .addCase(fetchRecommendedProducts.rejected, (state, action) => {
                state.isLoading = false;
                // state.error = action.payload || 'Failed to fetch recommended products';
              })
